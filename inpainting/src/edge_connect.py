...
#            num_workers=4,
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # inpaint model
                    ...
                    outputs, gen_loss, dis_loss, logs,gen_gan_loss = self.inpaint_model.process(images,edges.detach(), masks)
                    ...
                    logs.append(('gan_loss',gen_gan_loss.item()))
                ...
                # inpaint with edge model
                    ...
                    precision, recall = self.edgeacc(edges * masks, outputs * masks)
                    logs.append(('egen_loss',gen_loss.item()))
                    logs.append(('precision', precision.item()))
                    e_logs = logs

                    self.edge_model.backward(gen_loss,dis_loss)
                    outputs = outputs * masks + edges*(1.0 - masks)
    ...
    def sample(self, it=None):
        ...
        # inpaint model
            ...
            edges = self.edge_model(images_gray,edges,masks)
            outputs = self.inpaint_model(images, edges.detach(), masks)
            outputs_merged = (outputs * masks) + (images * (1 - masks))
        ...
        if model == 5:
            images = stitch_images(self.postprocess(outputs))
        else:
