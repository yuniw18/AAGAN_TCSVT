...
def test(model_path,test_path,epoch,DO_BEST,model_load):
    ...
    MB_Other_RESULT = test_path + '/epoch' + str(epoch) # Set results path according to ...
    ...
    if not DO_BEST:
        args.SAVED_MODEL = model_path + '/epoch' + str(epoch) + '.pth'
    else: 
        args.SAVED_MODEL = model_path + '/best.pth'
    ...
    if DO_MiddleBurryOther:
        ...
        for dir in subdir:
            ...
            log_path = MB_Other_RESULT + '/log.txt'
            ...
            f = open(log_path,'a')
            f.write(str(dir) + "\n")
            f.write( "interpolation error / PSNR : " + str(round(avg_interp_error_abs,4)) + " / " + str(round(psnr,4)) + "\n")
            f.write(metrics + "\n")
            f.close()
